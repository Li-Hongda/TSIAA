    ...
    def after_test_iter(self, runner, batch_idx: int, data_batch: DATA_BATCH = None, outputs: Optional[Sequence] = None) -> None:
        if runner.attacker.is_attack:
            ...
            if batch_idx == 0:
                ...
                self.image_path = f'{self.output_dir}/{self.dataset_name}/{self.attacker_name}_{self.model_name}_{runner.name}/images'
                self.label_path = f'{self.output_dir}/{self.dataset_name}/{self.attacker_name}_{self.model_name}_{runner.name}/labels'
